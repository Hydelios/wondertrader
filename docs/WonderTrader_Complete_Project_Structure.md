# WonderTrader 完整项目结构文档

> **版本**: v0.9.9
> **更新时间**: 2025-06-22
> **文档说明**: WonderTrader量化交易框架完整项目结构及文件功能说明
...
WonderTrader是一个基于C++核心的高性能量化交易框架，支持全市场全品种交易，具有以下特点：

- **🚀 高性能**: 系统延迟最低可达175纳秒
- **🔧 模块化**: 高度解耦的模块化设计
- **🌐 全品种**: 支持期货、期权、股票全品种交易
- **⚡ 多引擎**: CTA、SEL、HFT、UFT四种交易引擎
- **🔗 跨语言**: 通过Porter接口支持Python等语言
...
- **主要功能**:
  ...
  - 执行器管理与风控集成
- **关键组件**:
  - `WtEngine` - 引擎基类，提供统一生命周期管理
  - `WtCtaEngine` - CTA同步策略引擎（微秒级延迟）
  - `WtHftEngine` - HFT高频策略引擎（1-2微秒延迟）
  - `WtSelEngine` - SEL异步策略引擎（多因子选股）
  - `WtExecuterFactory` - 执行器工厂，支持多种执行算法

#### **WtUftCore - 超高频核心引擎**
- **作用**: 专为极速交易设计的超高频引擎
- **性能**: 系统延迟在200纳秒以内
- **特点**: 完全从WtCore剥离，不向应用层提供接口，全C++实现
- **适用场景**: 超高频交易、极速套利、市场做市
...
- **特点**:
  - 支持多语言策略统一回测
  - C++实现，回测效率高
  - 支持所有策略类型（CTA、SEL、HFT、UFT）
- **主要组件**:
  - `HisDataReplayer` - 历史数据回放器
  - `MatchEngine` - 撮合引擎
  - 各类策略模拟器（CtaMocker、HftMocker等）

### 2. 📊 数据管理模块

#### **WtDataStorage - 标准数据存储引擎**
- **作用**: 高性能本地数据存储，支持mmap文件映射
- **特点**: 针对传统交易时间设计，高效的数据读写
- **主要组件**:
  - `WtDataReader/Writer` - 数据读写器
  - `WtBtDtReader` - 回测数据读取器
  - `WtRdmDtReader` - 随机数据读取器

#### **WtDataStorageAD - 高级数据存储引擎**
- **作用**: 针对7x24小时交易市场优化的数据存储引擎
- **特点**: 支持数字货币等全天候交易市场
- **技术**: 基于LMDB数据库，支持高并发读写
...
#### **行情解析器系列 (ParserXXX)**
- **支持的行情源**:
  - **期货**: CTP、CTPMini、Femas、YD、XeleSkt
  - **期权**: CTPOpt、HuaX
  - **股票**: XTP、OES
  - **通用**: UDP、共享内存
- **特点**: 插件化设计，易于扩展新的行情源
...
#### **交易接口系列 (TraderXXX)**
- **支持的交易通道**:
  - **期货**: CTP、CTPMini、Femas、YD
  - **期权**: CTPOpt
  - **股票**: XTP、XTPXAlgo、ATP、OES、HuaX、STP
  - **特殊**: TraderMocker（本地仿真）、TraderDumper（数据转储）

### 4. 🔧 执行与风控系统

#### **WtExeFact - 执行器工厂**
- **执行算法**:
  - `WtMinImpactExeUnit` - 最小冲击执行
  - `WtTWapExeUnit` - 时间加权平均价格执行
  - `WtVWapExeUnit` - 成交量加权平均价格执行
  - `WtDiffMinImpactExeUnit` - 差量最小冲击执行
- **特点**: 支持多种执行算法，可根据市场情况选择最优执行策略
...
#### **策略工厂模块**
- **WtCtaStraFact**: CTA策略工厂，内置DualThrust策略
- **WtHftStraFact**: HFT策略工厂，提供高频交易策略模板
- **WtSelStraFact**: SEL策略工厂，支持多因子选股策略
- **WtUftStraFact**: UFT策略工厂，超高频策略模板
...
### **CMake构建系统**
- **主配置**: `src/CMakeLists.txt` - 整个项目的主CMake配置
- **平台支持**: Windows (MSVC) 和 Linux (GCC 8.4.0+)
- **C++标准**: C++17
- **依赖管理**: 通过环境变量配置依赖库路径

### **Visual Studio解决方案**
- `all.sln` - 完整解决方案，包含所有模块
- `backtest.sln` - 回测相关模块
- `datakit.sln` - 数据工具模块
- `parsers.sln` - 行情解析器模块
- `product.sln` - 产品核心模块
...
### **构建脚本**
- `build_debug.sh` - Linux调试版本构建
- `build_release.sh` - Linux发布版本构建
- `copy_bins_*.sh/bat` - 二进制文件复制脚本
...
### **运行器分发包**
- **LoaderRunner**: 合约加载器，用于从交易所API加载合约信息
- **QuoteFactory**: 行情工厂，负责行情数据的接入和存储
- **WtBtRunner**: 回测运行器，纯C++环境回测
- **WtRunnerCta**: CTA引擎运行器，同步策略交易
- **WtRunnerHft**: HFT引擎运行器，高频交易
- **WtRunnerUft**: UFT引擎运行器，超高频交易

### **配置文件系统**
- **common/**: 公共配置文件，包含合约、商品、手续费、节假日等配置
- **storage/**: 数据存储目录，包含二进制和CSV格式的历史数据
...
### **项目特点**
1. **高性能**: C++核心，系统延迟最低可达175纳秒
2. **全品种支持**: 期货、期权、股票全覆盖
3. **多引擎架构**: CTA、SEL、HFT、UFT四种引擎
4. **模块化设计**: 高度解耦，便于扩展和维护
5. **跨语言支持**: 通过Porter模块支持Python等语言

### **架构特色**
1. **M+1+N架构**: 多策略+统一执行+多账户
2. **信号执行分离**: 策略逻辑与执行逻辑解耦
3. **数据伺服架构**: 1+N数据服务，支持多客户端
4. **插件式扩展**: 策略、执行器、风控均支持插件化

### **应用场景**
1. **专业机构量化交易**: 完整的量化交易解决方案
2. **多账户管理**: 统一策略，多账户执行
3. **算法交易**: 独立执行器，支持各种执行算法
4. **极速交易**: UFT引擎，纳秒级延迟
5. **研究回测**: 高性能回测引擎，支持多语言策略

---

## 📝 总结

WonderTrader是一个功能完整、性能卓越的量化交易框架，其模块化的设计使得系统具有极高的可扩展性和可维护性。从数据接入到策略执行，从回测分析到风险控制，WonderTrader覆盖了量化交易的全流程，是专业级量化交易系统的理想选择。

**文档版本**: v1.0
**最后更新**: 2025-06-22
**维护者**: WonderTrader开发团队